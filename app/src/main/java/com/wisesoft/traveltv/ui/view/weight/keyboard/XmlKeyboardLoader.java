...
public class XmlKeyboardLoader {
	...
	public SoftKeyboard loadKeyboard(int resourceId) {
		...
		try {
				...
				if (mXmlEventType == XmlResourceParser.START_TAG) {
					...
					if (XMLTAG_KEYBOARD.compareTo(attr) == 0) { // keyboard
						...
						if (softKeyboard == null) {
							Lg.d(TAG,"XMLTAG_ROW softKeyboard null");
							return null;
						}
						...
						if (null == splitter || null == labels) {
							Lg.d(TAG,"XMLTAG_KEYS splitter or labels null");
							return null;
						}
						...
						if (null == softKeyboard) {
							Lg.d(TAG,"XMLTAG_KEY softKeyboard null");
							return null;
						}
			...
			Lg.d(TAG,"loadKeyboard load over");
			return softKeyboard;
		} catch (XmlPullParserException e) {
			e.printStackTrace();
			Lg.d(TAG,"loadKeyboard XmlPullParserException " + e.getMessage());
		} catch (IOException e) {
			e.printStackTrace();
			Lg.d(TAG,"loadKeyboard IOException " + e.getMessage());
		} finally {
			if (xrp != null)
				xrp.close();
		}
